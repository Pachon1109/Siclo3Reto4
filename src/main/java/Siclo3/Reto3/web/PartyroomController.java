...
public class PartyroomController {
    ...
    public Partyroom save(@RequestBody Partyroom p) {
        return partyroomService.SavePartyroom(p);  
        
    }
    @PutMapping("/update")
    @ResponseStatus(HttpStatus.CREATED)
    public Partyroom partyroomUpdate(@RequestBody Partyroom p) {
    return partyroomService.partyroomUpdate(p);
  }

    @DeleteMapping("{id}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public Boolean PartyroomDel(@PathVariable Integer id) {
    return partyroomService.PartyroomDel(id);
  }
}
